...
// Wire werkt naar toebehoren alle variabelen worden correct ontvangen.
// Stepper werkt samen met Wire om de benodigde biljetten door te krijgen.
// Bon Printer werkt samen met de wire informatie om die op goede plek te zetten.
...
//  int duur,
    int _step = 2;
  uint16_t stepsPerRev = 4096, stepsToTake;
 ...
 delay(1); 
    ...
    stepCount = 2;
      ...
      void dispense(int amount){
        stepsToTake = amount * 4096;
        Serial.println(stepsToTake);
      }

      boolean isDispensing(){
        return (bool)stepsToTake;
      }
};
...
//    printer.println("3011 WN Rotterdam");   //Postcode van de bank zodat de klant kan zien waar gepint is
    ...
    if (wantsReceipt == 1){
    printer.setSize('S');
    printer.println(addSpaces(dateTime.substring(0, 5), dateTime.substring(5, 16)));  // time    date
       
    printer.println("- - - - - - - - - - - - - - - -");   // seperating line
    printer.boldOff();    

    printer.println(addSpaces("Account : ", "XXXX XX" +bankAccount));  // iban
    ...
    }else if(wantsReceipt == 3){
      printer.justify('C');
      printer.println("Insuffient amount");
      printer.println("Try again with lower amount");
    }
